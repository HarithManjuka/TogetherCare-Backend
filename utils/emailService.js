...
/**
 * Sends a branded TogetherCare Account Email Verification OTP
 */
const sendAccountEmailVerificationOtp = async (toEmail, firstName, otp) => {
  const transporter = createTransporter();
  ...
  const htmlTemplate = `
    ...
    <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>TogetherCare Email Verification</title>
      </head>
      <body style="margin: 0; padding: 0; background-color: #f4f6f8; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif;">
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="table-layout: fixed;">
          <tr>
            <td align="center" style="padding: 40px 15px;">
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 520px; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 16px rgba(0,0,0,0.06);">
                ...
                <tr>
                  <td style="padding: 36px 32px 24px;">
                    <h2 style="color: #111827; font-size: 20px; font-weight: 700; margin: 0 0 12px;">Verify Your Email Address</h2>
                    <p style="color: #4B5563; font-size: 14px; line-height: 22px; margin: 0 0 24px;">
                      Hello ${firstName || 'Valued Member'},<br>
                      Please use the 4-digit verification code below to verify your email address on your TogetherCare account:
                    </p>

                    <!-- Code Block -->
                    <div style="background-color: #F0FDF4; border: 1.5px dashed #22C55E; border-radius: 12px; padding: 20px; text-align: center; margin-bottom: 24px;">
                      <span style="font-family: monospace, Courier; font-size: 36px; font-weight: 800; letter-spacing: 12px; color: #15803D; padding-left: 12px;">${otp}</span>
                    </div>
  ...
  await transporter.sendMail({
    from: process.env.EMAIL_FROM || '"TogetherCare Security" <togethercareadmin@gmail.com>',
    to: toEmail,
    subject: `TogetherCare: ${otp} is your email verification code`,
    html: htmlTemplate,
  });
};
...
module.exports = {
  ...
  sendAccountEmailVerificationOtp,
};